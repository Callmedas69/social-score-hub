...
contract SocialScoreHubNFT is ERC721, Ownable, Pausable, ReentrancyGuard {
    ...
    event Minted(address indexed to, uint256 indexed tokenId, bytes32 colorSeed);

    constructor(address initialOwner, address initialTreasury) ERC721("Social Score Hub", "SSH") Ownable(initialOwner) {
    ...
    function mint() external payable whenNotPaused nonReentrant {
        ...
        if (lastMint != 0 && block.timestamp < lastMint + MINT_COOLDOWN) {
            revert MintCooldownActive(lastMint + MINT_COOLDOWN - block.timestamp);
        }

        uint256 tokenId = _nextTokenId++;
        bytes32 seed = keccak256(abi.encodePacked(msg.sender, block.timestamp));
    ...
    function _getColors(bytes32 seed) internal pure returns (string[6] memory colors) {
    ...
    function _getDarkColors(bytes32 seed) internal pure returns (string[6] memory darkColors) {
        for (uint256 i = 0; i < 6; i++) {
            bytes3 colorBytes = bytes3(seed << (i * 24));
    ...
    function _generateSVG(string[6] memory colors, string[6] memory darkColors) internal pure returns (string memory) {
        return string(
            abi.encodePacked(
                _svgHeader(),
    ...
    function _svgDefs(string[6] memory colors, string[6] memory darkColors) internal pure returns (string memory) {
        return string(
            abi.encodePacked(
                "<defs>",
    ...
    function _svgGradients(string[6] memory colors, string[6] memory darkColors) internal pure returns (string memory) {
        return string(
            abi.encodePacked(
    ...
    function _gradient(string memory id, string memory light, string memory dark) internal pure returns (string memory) {
        return string(
            abi.encodePacked(
    ...
    function _orb(string memory cx, string memory cy, string memory gradientId) internal pure returns (string memory) {
        return string(
            abi.encodePacked(
                '<circle cx="', cx, '" cy="', cy, '" r="40" fill="url(#', gradientId, ')" filter="url(#g)"/>',
                '<circle cx="', cx, '" cy="', cy, '" r="20" fill="#FFF" filter="url(#c)"/>'
            )
        );
    }

    function _generateAttributes(string[6] memory colors) internal pure returns (string memory) {
        return string(
            abi.encodePacked(
